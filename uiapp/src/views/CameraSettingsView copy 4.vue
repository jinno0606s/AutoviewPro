...
async function onTogglePeople(cam){
  ...
  const newValue =
    cam.people_count_enabled === 1 ? 0 : 1

  cam.people_count_enabled = newValue
  ...
  if(window.autoview){
    ...
    await api.post("/api/cameras/update", payload)
  }
    ...
    <table>
      ...
      <tbody>
        <tr v-for="cam in cameras" :key="cam.id">
          ...
          <td>
            <button @click="openPtz(cam)">PTZ</button>
          </td>
