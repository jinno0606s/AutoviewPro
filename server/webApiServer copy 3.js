...
function readEventsFromCsv() {
  ...
  for (let i = 1; i < lines.length; i++) {
    const [ts, serial, mode, type, direction, value] = lines[i].split(",")
...
//////////MQTT webapiserver.js/////////////////////////////////////////
    // MQTTイベント集計API AOA 交差点横断集計 0420 651
    if (req.url.startsWith("/api/people-summary")) {

      const url = new URL(req.url, "http://localhost")

      const mode = url.searchParams.get("mode") || "cross"
    ...
    if (req.url.startsWith("/api/events-agg-timeseries")) {
      ...
      const from = new Date(url.searchParams.get("from")).getTime()
      const to = new Date(url.searchParams.get("to")).getTime()
      const unit = url.searchParams.get("unit") || "minute"

      //const events = readEventsFromCsv() // ← ★変更
      const cam = url.searchParams.get("cam")

      const events = readEventsFromCsv().filter(e => {
        if (!cam) return true
        return e.serial === cam
      })
